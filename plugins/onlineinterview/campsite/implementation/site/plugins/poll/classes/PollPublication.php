...
class PollPublication extends DatabaseObject {
     ...
     * @var array
     */
    var $m_keyColumnNames = array('fk_poll_nr', 'fk_poll_language_id', 'fk_publication_id');
     ...
     */
    function PollPublication($p_fk_poll_language_id = null, $p_fk_poll_nr = null, $p_fk_publication_id = null)
    {
    ...
    {      
        foreach (PollPublication::getAssignments($p_fk_poll_nr, $p_fk_poll_language_id) as $record) {
            $record->delete();   
        }   
    }
     ...
     * @param int $p_fk_publication_id
     */
    public static function OnPublicationDelete($p_fk_publication_id)
    {      
        foreach (PollPublication::getAssignments(null, null, $p_fk_publication_id) as $record) {
            $record->delete();   
        }   
    }
                                            ...
                                            $p_offset = 0, $p_limit = 10, $p_orderStr = null)
    {
        ...
        while ($row = $res->fetchRow()) {
            $records[] =& new PollPublication($row['fk_poll_language_id'], $row['fk_poll_nr'], $row['fk_publication_id']);      
        } 
