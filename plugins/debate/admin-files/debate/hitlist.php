...
<TR>
    <TD COLSPAN="2">
        <B><?php  if ($debate) putGS("Edit Debate"); else putGS('Add new Debate'); ?></B>
    ...
    <td valign="top">
        ...
        for ($n=1; $n<=20; $n++) {
            ...
            <tr id="debate_answer_tr_<?php p($n); ?>" style="display: <?php $nr_of_answers >= $n ? p('table-row') : p('none'); ?>">
                ...
                <TD>
                <INPUT TYPE="TEXT" NAME="f_answer[<?php p($n); ?>]" SIZE="40" MAXLENGTH="255" class="input_text" alt="blank" id="debate_answer_input_<?php p($n); ?>" emsg="<?php putGS('You must fill in the $1 field.', getGS('Answer $1', $n)); ?>" value="<?php isset($answers[$n]) ? p(htmlspecialchars($answers[$n])) : p('__undefined__'); ?>">
                </TD>
