    ...
    <tr  class="<?= $index % 2 ? 'odd' : 'even' ?>">
      <td>
          <div class="soundcloud-list-item">
            <div class="buttons">
            <a id="<?= $t['id'] ?>" href="" class="track-play ui-state-default icon-button"><span class="ui-icon"></span><? echo $translator->trans('Play', array(), 'plugin_soundcloud') ?></a>
            <? if ($g_user->hasPermission('plugin_soundcloud_update')): ?>
                <a id="<?= $t['id'] ?>" href="" class="track-edit ui-state-default icon-button"><span class="ui-icon ui-icon-pencil"></span><? echo $translator->trans('Edit') ?></a>
                <a id="<?= $t['id'] ?>" href="/<?= $ADMIN ?>/soundcloud/controller.php?action=setlist&track=<?= $t['id'] ?>" class="track-set ui-state-default icon-button"><span class="ui-icon"></span><? echo $translator->trans('Add to set', array(), 'plugin_soundcloud') ?></a>
               <? endif ?>
            ...
            <div class="metadata">
                ...
                <?= $t['description'] ?>
                <div class="info"><?= $translator->trans('Tags', array(), 'plugin_soundcloud') . ': ' .  $t['tag_list'] ?></div>
                ...
                <? if($t['sharing'] != 'public'): ?>
                    | <img alt="<?= $translator->trans('Private', array(), 'plugin_soundcloud') ?>" src="<?= $Campsite['SUBDIR'] ?>/plugins/soundcloud/css/images/locked_big.png">
                <? endif ?>
                </span>
            </div>
        </div>
...
<div class="fg-toolbar ui-toolbar ui-widget-header ui-corner-bl ui-corner-br ui-helper-clearfix">
  <div class="dataTables_info" id="gridx_info"><? echo $translator->trans('Showing $1 to $2', array('$1' => $counter == 0?$offset:$offset+1, '$2' => $offset+$counter), 'plugin_soundcloud') ?></div>
  ...
  <a id="search-prev" class="fg-button ui-button ui-state-default ui-corner-left <?= $offset == 0 ? 'ui-state-disabled' : '' ?>" title="<? echo $translator->trans('Previous', array(), 'plugin_soundcloud') ?>" id="gridx_previous"><span class="ui-icon ui-icon-circle-arrow-w"></span></a>
  <a id="search-next"class="fg-button ui-button ui-state-default ui-corner-right" title="<? echo $translator->trans('Next', array(), 'plugin_soundcloud') ?>" id="gridx_next"><span class="ui-icon ui-icon-circle-arrow-e"></span></a></div>
</div>
<? else: ?>
    <? echo $translator->trans('No tracks found', array(), 'plugin_soundcloud') ?>
<? endif ?>
