 ...
 * @package Campsite
 */
class PollAnswerAttachmentsList extends ListObject 
{                                 
    private $m_item;
	...
	protected function ProcessConstraints(array $p_constraints)
	{
	    ...
	    foreach ($p_constraints as $word) {
	        ...
	        if ($state == 2) {
	                $type = PollAnswerAttachmentsList::$s_parameters[$attribute];
	                try {
	    ...
	    foreach ($p_order as $word) {
	        switch ($state) {
                case 1: // reading the order field
	                if (array_search(strtolower($word), PollAnswerAttachmentsList::$s_orderFields) === false) {
