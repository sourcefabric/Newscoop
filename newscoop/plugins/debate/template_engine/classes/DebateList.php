 ...
 *  @package Campsite
 */
class DebateList extends ListObject
{
    private $m_item;

    public static $s_parameters = array(
        'number' => array('field' => 'debate_nr', 'type' => 'integer'),
        'language_id' => array('field' => 'fk_language_id', 'type' => 'integer'),
        'parent_debate_nr' => array('field' => 'parent_debate_nr', 'type' => 'integer'),
	...
	protected function CreateList($p_start = 0, $p_limit = 0, array $p_parameters, &$p_count)
	{
	    ...
	    foreach ($debatesList as $debate) {
	        $metaDebatesList[] = new MetaDebate($debate->getLanguageId(), $debate->getNumber(), $context->user->id);
	    }
	    ...
	    foreach ($p_constraints as $word) {
	        switch ($state) {
	            case 1: // reading the parameter name
	                if (!array_key_exists($word, DebateList::$s_parameters)) {
	                    CampTemplate::singleton()->trigger_error("invalid attribute $word in list_debates, constraints parameter");
	                    break;
	                }
	                $attribute = $word;
	                $state = 2;
	            ...
	            case 2: // reading the operator
	                $type = DebateList::$s_parameters[$attribute]['type'];
	                try {
	                    $operator = new Operator($word, $type);
	                }
	                catch (InvalidOperatorException $e) {
	                    CampTemplate::singleton()->trigger_error("invalid operator $word for attribute $attribute in list_debates, constraints parameter");
	                }
	                $state = 3;
	                break;
	            case 3: // reading the value to compare against
	                ...
	                try {
	                    ...
	                    CampTemplate::singleton()->trigger_error("invalid value $word of attribute $attribute in list_debates, constraints parameter");
	                }
	                $state = 1;
	                break;
	        }
	...
	protected function ProcessOrder(array $p_order)
	{
	    ...
	    foreach ($p_order as $word) {
	        switch ($state) {
                case 1: // reading the order field
	                if (array_search(strtolower($word), DebateList::$s_orderFields) === false) {
	                    CampTemplate::singleton()->trigger_error("invalid order field $word in list_debates, order parameter");
	                } else {
    	                $orderField = $word;
	                }
	                $state = 2;
                ...
                case 2: // reading the order direction
                    if (MetaOrder::IsValid($word)) {
                        ...
                        CampTemplate::singleton()->trigger_error("invalid order $word of attribute $orderField in list_debates, order parameter");
                    }
                    $state = 1;
	                break;
	        }
	    ...
	    if ($state != 1) {
            CampTemplate::singleton()->trigger_error("unexpected end of order parameter in list_debates");
	    }
    			...
    			case 'item':
    				if ($parameter == 'length' || $parameter == 'columns') {
    					...
    					if ("$intValue" != $value || $intValue < 0) {
    						CampTemplate::singleton()->trigger_error("invalid value $value of parameter $parameter in statement list_debates");
    					}
    				...
    				break;
    			default:
    				CampTemplate::singleton()->trigger_error("invalid parameter $parameter in list_debates", $p_smarty);
    		}
    	}
