...
class SubscriptionsNotifierCommand extends Console\Command\Command
{
    ...
    {
        $this->em = $this->getHelper('container')->getService('em');

        $etc_directory = APPLICATION_PATH . '/../conf/';
        $notifierTemplate = '_subscription_notifier.tpl';
        $message = array();
        $notifiedIndex = false;

        if (!FilesystemService::isReadable($etc_directory . '/install_conf.php')) {
            exit;
        }

        require_once($etc_directory . '/install_conf.php');
        require_once(APPLICATION_PATH . '/../include/campsite_init.php');

        if (!is_file($etc_directory . '/database_conf.php')) {
        ...
        foreach ($endingSubscriptions as $subscription) {
            ...
            foreach ($subscritpionsSections as $subscriptionSection) {
                ...
                $remained_days = $paidDays + $toDaysStartDate - $toDaysNow;
                ...
                if ($counter == 0) {
                    $subs_type = ($subscription->getType() == 'P') ? 'paid' : 'trial';
                ...
                if ($subs_sections == $sectionsCount && count($subscritpionsSections) == 1) {
        ...
        if ($notified_index > 0) {
            if ($input->getOption('verbose')) {
                $output->writeln('<info>'.$notified_index . ' user(s) notified.<info>');
            }
        }
    }
