<?php 
regGS("If you continue, topic and all subtopics will be detached from all articles, and deleted.", "Si continúa, los temas y subtemas serán desligados de sus artículos y eliminados.");
regGS("Delete anyway", "Eliminar de todas formas");
regGS("Reset order", "Restablecer el orden");
regGS("Order changes will be lost. Are you sure you want to continue?", "Se perderan los cambios de orden.  ¿Está seguro de continuar?");
regGS("Topics order saved.", "Se guardó el órden de los tópicos");
regGS("Show languages", "Mostrar idiomas");
regGS("Save order", "Guardar orden");
regGS("Click to edit", "Clic para editar");
regGS("Add root topic", "Añadit tópico raíz");
regGS("Topic was deleted.", "Se eliminó el tópico.");
regGS("The topic name is already in use by another topic.", "Este nombre de tópico ya es usado por otro tópico.");
regGS("You must enter a name for the topic.", "Debe ingresar un nombre para el tópico");
regGS("You must choose a language for the topic.", "Debe elegir un idioma para el tópico");
regGS("This topic has subtopics, therefore it cannot be deleted.", "Este tópico tiene subtópicos, por lo que no puede ser eliminado");
regGS("Add translation:", "Agregar traducción:");
regGS("Add subtopic:", "Agregar subtópico:");
regGS("Top", "Arriba");
regGS("You do not have the right to add topics.", "No tiene derecho para agregar temas");
regGS("You do not have the right to delete topics.", "No tiene derecho para borrar temas");
regGS("Are you sure you want to delete the topic $1?", "¿Esta seguro que desea borar el tema $1?");
regGS("You do not have the right to change topic name.", "No tiene derecho para cambiar nombre de tema");
regGS("The topic $1 could not be deleted.", "El tema $1 no pudo ser borrado");
regGS("Adding new topic", "Agregando nuevo tema");
regGS("Deleting topic", "Borrando tema");
regGS("Change topic name", "Cambiar nombre de tema");
regGS("There are $1 articles using the topic.", "Hay $1 articulo(s) usando el tema");
?>