<?php 
regGS("so that default templates can be assigned to the issue.", "para que las plantillas por defecto puedan asignarse a la edición.");
regGS("Once this is done, the issue can be published", "Será posible publicar la edición una vez completado este paso.");
regGS("This issue cannot be previewed. Please make sure the publication has a theme assigned.", "No es posible previsualizar esta edición.  Por favor asegúrese de que la publicación tiene un tema asignado.");
regGS("This issue cannot be previewed. Please make sure it has the front template selected.", "No es posible previsualizar esta edición.  Por favor asegúrese de que ha seleccionado una plantilla para la portada.");
regGS("Publish all articles:", "Publicar todos los artículos:");
regGS("Publish all articles", "Publicar todos los artículos");
regGS("Force publishing of all articles. If set to 'No', only articles with 'Publish with Issue' status will be published.", "Forzar la publicación de todos los artículos.  Si tienen la configuración \"no\", solo se publicarán los marcados con \"Publicar con edición\".");
regGS("default", "por defecto");
regGS("The publishing/unpublishing can not be set the same time", "La opción de publicar/dejar de publicar no se puede determinar al mismo tiempo");
regGS("The publishing schedule can not be set in the past", "La programación de una publicación no se puede hacer en una fecha pasada");
regGS("Please assign at least one theme to the publication", "Por favor asigne al menos un tema a la publicación");
regGS("Issue Theme", "Tema de la edición");
regGS("The values you are trying to set conflict with issue \"$1$2. $3 ($4)$5\".", "Los valores que intenta establecer tienen conflicto con la edición \"$1$2. $3 ($4)$5\".");
regGS("The number must be unique for each issue in this publication of the same language.", "Cada edición de esta publicación del mismo idioma debe tener un número único.");
regGS("The language and URL name must be unique for each issue in this publication.", "El idioma y el nombre del URL deben ser únicos para cada edición en esta publicación.");
regGS("Issue created.", "Se creó una nueva edición.");
regGS("Issue updated", "Se actualizó la edición.");
regGS("Could not save the changes to the issue.", "No fue posible almacenar los cambios a la edición.");
regGS("Unpublish on: $1", "Despublicar el:  $1");
regGS("There are $1 articles in this issue.", "Hay $1 artículos en esta edición");
regGS("The issue $1 has been deleted.", "Se ha eliminado la edición $1");
regGS("Schedule", "Programar");
regGS("Publish on: $1", "Publicar el: $1");
regGS("Publish Date $1", "Fecha de publicación $1");
regGS("Deleted issue", "Edición eliminada");
regGS("Delete issue", "Eliminiar edición");
regGS("Click here to publish this issue", "Pinche aquí para publicar esta edición");
regGS("A total of $1 articles were deleted.", "Se eliminó un total de $1 artículos");
regGS("Name<BR><SMALL>(click to see sections)</SMALL>", "Nombre<BR><SMALL>(clic para ver secciones)</SMALL> ");
regGS("Delete issue $1", "Borrar edición $1");
regGS("You do not have the right to change issues.", "No tiene autorización para cambiar ediciones.");
regGS("Are you sure you want to change the issue $1 status from $2 to $3?", "¿está seguro que desea cambiar el status de la edición $1 de $2 a $3?");
regGS("Add new translation", "Agregar nueva traducción");
regGS("You do not have the right to add issues.", "No tiene autorización para agregar ediciones");
regGS("The issue $1 has been successfuly added.", "La edición $1 ha sido agregada con éxito");
regGS("The issue could not be added.", "La edición no pudo ser agregada");
regGS("Change issue details", "Cambiar detalles de la edición");
regGS("You do not have the right to change issue details.", "No tiene autorización para cambiar detalles de la edición");
regGS("Publication date<BR><SMALL>(yyyy-mm-dd)</SMALL>", "Fecha de Publicación<BR><SMALL>(yyyy-mm-dd)</SMALL> ");
regGS("Front Page Template", "Plantilla de portada");
regGS("You do not have the right to delete issues.", "No tiene privilegios para borrar ediciones");
regGS("Are you sure you want to delete the issue $1?", "¿Esta seguro que desea borrar la edición $1?");
regGS("Use the structure of the previous issue", "Usar la estructura de la edición previa");
regGS("Copy the entire structure in all languages from the previous issue except for content.", "Copiar la estructura completa de la edición previa en todos los idiomas exceptuando el contenido");
regGS("You may modify it later if you wish.", "Puede modificarla luego si desea");
regGS("Create a new structure", "Crear una nueva estructura");
regGS("Create a complete new structure.", "Crear una nueva estructura completa");
regGS("You must define an issue type for each language and then sections for them.", "Debe definir un tipo edición para cada idioma y secciones para ella");
regGS("Copy previous issue", "Copiar edición previa");
regGS("No previous issue.", "No existen ediciones previas");
regGS("Copy structure from issue number $1", "Copiar la estructura de la edición número $1");
regGS("Delete entry", "Eliminar registro");
regGS("Issue Publishing Schedule", "Horario de publicación de ediciones");
regGS("Invalid Input: $1", "Registro inválido:  $1");
regGS("Issue Number", "Edición número");
regGS("Go To Sections", "Ir a secciones");
regGS("There was an error reading the language parameter.", "Hubo un error en la lectura del parámetro de idioma.");
regGS("There was an error reading the publication parameter.", "Hubo un error en la lectura del parámetro de publicación.");
regGS("There was an error reading the issue parameter.", "Hubo un error en la lectura del parámetro de edición.");
regGS("There was an error scheduling the $1 action on $2", "Hubo un error en la programación de la acción $1 para el $2");
regGS("The action scheduled on $1 could not be deleted.", "No se pudo eliminar la acción programada para el $1.");
regGS("Front Page Template<BR><SMALL>(click to change)</SMALL>", "Plantilla de Portada");
regGS("Single Article Template<BR><SMALL>(click to change)</SMALL>", "Plantilla de Artículos Individuales");
regGS("Changing issue status", "Cambiando estatus de edición");
regGS("Status of the issue $1 has been changed from $2 to $3", "Estatus de edición $1 ha sido cambiado de $2 a $3");
regGS("Status of the issue $1 could not be changed.", "Estatus de edición $1 no pudo ser cambiado");
regGS("Updating issue", "Actualizando edición");
regGS("Changing issue's details", "Cambiando detalles de edición");
regGS("The issue $1 has been successfuly changed.", "La edición $1 ha sido cambiada con éxito");
regGS("The issue could not be changed.", "La edición no pudo ser cambiada");
regGS("Preview issue", "Vista previa de edición");
regGS("This issue cannot be previewed. Please make sure it has a $1 template selected.", "La edición no puede ser pre-vista.  Favor asegurese de que tiene una plantilla de $1 seleccionada");
regGS("front page", "portada");
regGS("Changing issue template", "Cambiando plantilla de edición");
regGS("You do not have the right to change issue templates.", "No tiene derecho a cambiar plantillas de edición");
regGS("Changing issue template for front page", "Cambiando plantilla de edición para portada");
regGS("Changing issue template for single article", "Cambiando plantilla de edición para artículos individuales");
regGS("The template has been successfully changed.", "La plantilla ha sido cambiada con éxito");
regGS("Issue template for publication $1 changed to $2", "Plantilla de edición para publicación $1 cambió a $2");
regGS("Issue template for single articles from $1 changed to $2", "Plantilla de edición para artículos individuales $1 cambió a $1");
regGS("The template could not be changed.", "La platilla no pudo ser cambiada");
?>