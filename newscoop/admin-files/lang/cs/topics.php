<?php 
regGS("If you continue, topic and all subtopics will be detached from all articles, and deleted.", "Pokud budete pokračovat, bude téma a všichni podtémata oddělena od všech článků a smazána.");
regGS("Delete anyway", "Stejně odstranit");
regGS("Reset order", "Přenastavit pořadí");
regGS("Order changes will be lost. Are you sure you want to continue?", "Požadované změny budou ztraceny. Jste si jisti, že chcete pokračovat?");
regGS("Topics order saved.", "Pořadí témat uloženo.");
regGS("Show languages", "Ukázat jazyky");
regGS("Save order", "Uložit pořadí");
regGS("Click to edit", "Klikněte pro možnost úpravy");
regGS("Add root topic", "Přidat zdrojové téma");
regGS("Topic was deleted.", "Téma bylo odstraněno.");
regGS("The topic name is already in use by another topic.", "Název tématu se již používá.");
regGS("You must enter a name for the topic.", "Zadejte název tématu.");
regGS("You must choose a language for the topic.", "Zvolte jazyk tématu.");
regGS("This topic has subtopics, therefore it cannot be deleted.", "Téma obsahuje podtémata, proto nemůže být odstraněno.");
regGS("Add translation:", "Přidat překlad:");
regGS("Add subtopic:", "Přidat témata:");
regGS("Top", "Nahoře");
regGS("You do not have the right to add topics.", "Nemáte právo přidat témata");
regGS("You do not have the right to delete topics.", "Nemáte právo mazat témata");
regGS("Are you sure you want to delete the topic $1?", "Určitě chcete smazat téma $1?");
regGS("You do not have the right to change topic name.", "Nemáte právo měnit název tématu");
regGS("The topic $1 could not be deleted.", "Téma $1 nemohlo být smazáno");
regGS("Adding new topic", "Přidání nového témata");
regGS("Deleting topic", "Mazání tématu");
regGS("Change topic name", "Změnit název témata");
regGS("There are $1 articles using the topic.", "$1 článků používá toto téma");
?>