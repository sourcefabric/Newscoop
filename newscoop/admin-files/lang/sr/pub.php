<?php 
regGS("click to see issues", "кликните да видите издања");
regGS("The site alias for publication \"$1\" has been modified to \"$2\".", "Сајт алиас за публикацију \"$1\" је промењен у \"$2\".");
regGS("The site alias \"$1\" has been added to publication \"$2\".", "Сајт алиас \"$1\" је додат публикацији \"$2\".");
regGS("The alias \"$1\" has been deleted from publication \"$2\".", "Алиас \"$1\" је обрисан из публикације \"$2\".");
regGS("Subscription default time for \"$1\":$2 deleted", "Подразумевано трајање претплате за  \"$1\":$2 је обрисано");
regGS("Default subscription time for \"$1\":$2 changed", "Подразумевано трајање претплате за  \"$1\":$2 је промењено");
regGS("Moderator Address", "Адреса модератора");
regGS("From Address", "Адреса са које се шаље");
regGS("Use CAPTCHA to prevent spam?", "Користи CAPTCHA за превенцију спама?");
regGS("Public allowed to comment?", "Опште право коментарисања?");
regGS("Enable spam blocking?", "Омогући блокирање спама?");
regGS("The subscription settings for '$1' could not be added.", "Подешавања претплате за '$1' немогу бити додата");
regGS("The publication name you specified conflicts with publication '$1'.", "Име публикације које сте навели је у конфликту са публикацијомн '$1'.");
regGS("The publication alias you specified conflicts with publication '$1'.", "Алиас публикације који сте навели је у конфликту са публикацијом '$1'.");
regGS("Subscription defaults", "Подразумеване вредности за претплату");
regGS("Subscription Settings by Country", "Подешавање претплате по државама");
regGS("Set subscription settings for a country", "Постави подешавања претплате за државу");
regGS("Set subscription settings by country", "Подеси податке о претплати по државама");
regGS("Default time periods:", "Подразумевани временски период:");
regGS("Country subscription settings updated.", "Промењена су подешавања претплате за државу.");
regGS("Country subscription settings deleted.", "Обрисана су подешавања претплате за државу.");
regGS("Change country subscription settings", "Промени подешавања претплате за државу");
regGS("Are you sure you want to delete the subscription settings for $1?", "Да ли сте сигурни да желите да обришете подешавања претплате за $1?");
regGS("Publication updated", "Публикација измењена");
regGS("Subscriber comments moderated?", "Коментари претплатника се морају прво одобрити?");
regGS("Public comments moderated?", "Коментари се прво морају одобрити?");
regGS("Comments enabled?", "Омогући коментаре?");
regGS("Comments enabled", "Коментари омогућени");
regGS("Article comments default to enabled?", "Подразумевај да су коментари на текстове омогућени?");
regGS("trial subscription", "пробна претплата");
regGS("paid subscription", "плаћена претплата");
regGS("time units", "временске јединице");
regGS("one language", "један језик");
regGS("all languages", "сви језици");
regGS("Trial subscriptions", "Пробне претплате");
regGS("Time unit cost per one section", "Цена по временској јединици за рубрику");
regGS("Paid subscriptions", "Плаћене претплате");
regGS("Default time period", "Подразумевани временски период");
regGS("Go To Issues", "Иди на издања");
regGS("Delete publication $1", "Обриши публикацију $1");
regGS("No publications.", "Нема публикација");
regGS("Default Site Alias", "Основни сајт алиас");
regGS("URL Type", "тип УРЛа");
regGS("Country<BR><SMALL>(click to edit)</SMALL>", "Држава<br><small>(кликните за уређивање)</small>");
regGS("Delete alias $1", "Обриши податак $1");
regGS("Configure publication", "Подеси публикацију");
regGS("You do not have the right to edit publication information.", "Немате право да уређујете податке о публикацији");
regGS("General attributes", "Општи атрибути");
regGS("You do not have the right to change publication information.", "Немате право измене података о публикацији");
regGS("The publication information could not be updated.", "Подаци о публикацији не могу бити измењени");
regGS("You do not have the right to delete publications.", "Немате право да бришете публикације");
regGS("Are you sure you want to delete the publication $1?", "Сигурни сте да желите да обришете публикацију $1?");
regGS("Deleting publication", "Брисање публикације");
regGS("There are $1 subscription(s) left.", "Преостало је $1 претплата");
regGS("The publication $1 could not be deleted.", "Публикација $1 не може бити обрисана");
regGS("The publication could not be added.", "Публикација не може бити додата");
regGS("You do not have the right to manage publications.", "Немате право да администрирате публикацијама");
regGS("You must select a country.", "Морате изабрати државу");
regGS("Add new alias", "Додај нови алиас");
regGS("Alias (click to edit)", "Алиас (клик за уређивање)");
regGS("Edit aliases", "Уреди алиасе");
regGS("Publication Aliases", "алиаси за публикацију");
regGS("Adding new alias", "Додавање новог алиаса");
regGS("The site alias $1 could not be added.", "Сајт алиас $1 не може бити додат.");
regGS("Another alias with the same name exists already.", "Већ постоји алиас са истим именом");
regGS("Edit alias", "Уреди алиас");
regGS("Editing alias", "Уређивање алиаса");
regGS("The site alias $1 could not be modified.", "Сајт алиас $1 не може бити промењен.");
regGS("Are you sure you want to delete the alias $1?", "Да ли сте сигурни да желите да обришете алиас $1?");
regGS("Deleting alias", "Брисање алиаса");
regGS("The alias $1 could not be deleted.", "Алиас $1 не може бити обрисан.");
regGS("$1 is the default publication alias, it can not be deleted.", "$1 је основни алиас за публикацију, па не може бити обрисан.");
regGS("You do not have the right to add publications.", "Немате право да додајете публикације.");
regGS("There are $1 issue(s) left.", "Преостало је $1 издања.");
regGS("Time Unit", "Временска јединица");
regGS("Currency", "Валута");
regGS("Edit languages", "Прикажи језике");
regGS("Search engine optimization", "Оптимизација претраживача");
regGS("Article title", "Наслов текста");
regGS("Article keywords", "Кључне речи");
regGS("Article topics", "Теме");
regGS("Invalid URL Template", "Шаблон за приказ при погрешном УРЛу");
regGS("The default subscription time for $1 has been added.", "Нови период претплате за $1 додат");
?>