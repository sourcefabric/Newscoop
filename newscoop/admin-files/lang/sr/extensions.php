<?php 
regGS("Wikipedia Search", "");
regGS("Files", "");
regGS("Sourcefabric.org newsletter subscription", "Пријава на sourcefabric.org newsletter");
regGS("Maps search", "Претрага мапа");
regGS("Default location", "Подразумевана локација");
regGS("Title", "Наслов");
regGS("Sourcefabric.org feed reader", "Sourcefabric.org feed reader");
regGS("Sourcefabric.org dev feed reader", "Sourcefabric.org dev feed reader");
regGS("Recently Published Articles", "Скоро објављени текстови");
regGS("Recently Modified Articles", "Скоро мењани текстови");
regGS("Pending Articles", "Текстови на чекању");
regGS("Paste google generated code", "Залепи код који је генерисао гугл");
regGS("Most Popular Articles", "Најпопуларнији текстови");
regGS("Media files", "Медиа фајлови");
regGS("Hello world!", "Здраво свете!");
regGS("Hello World!", "Здраво свете!");
regGS("Google gadget", "Google справица");
regGS("Feed url", "Адреса feeda");
regGS("Access Denied", "Приступ забрањен");
?>