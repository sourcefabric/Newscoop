 ...
 * @link       http://hybridauth.sourceforge.net/userguide/IDProvider_info_PayPal.html
...
class Hybrid_Providers_PayPal extends Hybrid_Provider_Model
{
	...
	* begin login step
	* 
	* build an normalized PayPal OpenID url to autentify the user with selected openid_identifier
	*/
	function loginBegin()
	{
		...
		{
			throw new Exception( "Authentification failed! Missing reuqired parameter [openid_identifier] for PayPal" );
		}
	...
	* finish login step
	* 
	* fetch returned parameters by PayPal OpenID IDp 
	*/
	function loginFinish()
	{
		...
		{
			throw new Exception( "Authentification failed! The PayPal provider has returned an invalid response." );
		} 
		...
		{
			throw new Exception( "Authentification failed! PayPal verification failed: " . $response->message );
	    }
		...
		elseif( $response->status == Auth_OpenID_SUCCESS )
		{
			...
			{
				throw new Exception( "Authentification failed! User has accepted the authentfication, but the PayPal provider [{$this->openidIdentifier}] has not sent the user data." );
			}
	...
	*/
	function getUserProfile()
	{
		...
		Hybrid_Auth::addWarning( "PayPal provide the user profile only once" );
	}
