...
/**
 * Hybrid_Providers_OpenID class, wrapper for all OpenID providers 
 *
 * @package    Hybrid_Auth 
 * @author     Zachy <hybridauth@gmail.com>
 * @version    1.0
 * @since      HybridAuth 1.0.1  
 * @link       http://hybridauth.sourceforge.net/userguide/IDProvider_info_OpenID.html
...
class Hybrid_Providers_OpenID extends Hybrid_Provider_Model
{
	...
	*/
	function initialize() 
	{
		...
		# set require openid_identifier parameter,
		# openid_identifier must be defined when Hybrid_Auth::setup( $IDp, $params ) is called
		if( ! $this->params["openid_identifier"] )
		{ 
			throw new Exception( "Authentification failed! Missing reuqired parameter [openid_identifier] for OpenID provider" );
		} 
	...
	* begin login step
	* 
	* build an normalized OpenID url to autentify the user with selected openid_identifier
	*/
	function loginBegin()
	{
	...
	* finish login step
	* 
	* fetch returned parameters by selected OpenID IDp 
	*/
	...
	*/
	function getUserProfile()
	{
		...
		Hybrid_Auth::addWarning( "OpenID provide the user profile only once" );
	}
