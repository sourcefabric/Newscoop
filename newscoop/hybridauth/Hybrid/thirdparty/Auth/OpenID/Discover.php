...
Hybrid_Logger::info( "Auth_OpenID_discover", array( $uri, $fetcher ) );

    // If the fetcher (i.e., PHP) doesn't support SSL, we can't do
    // discovery on an HTTPS URL.
    if ($fetcher->isHTTPS($uri) && !$fetcher->supportsSSL()) {
	
	Hybrid_Logger::error ( "Auth_OpenID_discover If the fetcher (i.e., PHP) doesn't support SSL, we can't do" );
    ...
    if (Auth_Yadis_identifierScheme($uri) == 'XRI') {
        $result = Auth_OpenID_discoverXRI($uri, $fetcher);
			...
			Hybrid_Logger::info( "Auth_OpenID_discover Auth_OpenID_discoverXRI result", array( $result ) );

    } else {
        $result = Auth_OpenID_discoverURI($uri, $fetcher);
		
			Hybrid_Logger::info( "Auth_OpenID_discover Auth_OpenID_discoverURI result", array( $result ) );
    }
	...
	Hybrid_Logger::info( "Auth_OpenID_discover result", array( $result ) );
