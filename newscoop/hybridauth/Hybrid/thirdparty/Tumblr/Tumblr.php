...
class TumblrOAuth {/*{{{*/
  ...
  public static $TO_API_ROOT = "http://www.tumblr.com/";
   ...
   */
  function getAccessToken($token = NULL, $oauth_verifier ) {/*{{{*/
	Hybrid_Logger::debug( "getAccessToken  ", array($token , $oauth_verifier ) );
    $r = $this->oAuthRequest($this->accessTokenURL(), array( "oauth_verifier" => $oauth_verifier ), "POST" );
    $token = $this->oAuthParseResponse($r);
    $this->token = new OAuthConsumer($token['oauth_token'], $token['oauth_token_secret']);
    return $token;
  }/*}}}*/
  ...
  function http($url, $post_data = null) {/*{{{*/
	...
	Hybrid_Logger::debug( "Make an HTTP request", array( $url, $post_data, $response ) );
