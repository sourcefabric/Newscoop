...
ORDER BY Number DESC";
    ...
    public function assignThemeToPublication(Newscoop\Entity\Theme $theme, $publicationId)
    {
        ...
        // fixing same dir includes
        // @todo themesPath is actually unassigned themes directory
        $this->fixIncludesFromSamePath(rtrim(dirname($this->themesPath),DIR_SEP).DIR_SEP.$theme->getPath());
        return $this->getThemeService()->assignTheme($theme, $publication);
    }

    /**
     * non recursive directory scan for fixing *.tpl files
     * removing the current dirname from include declarations
     *
     * @param string $dir
     */
    private function fixIncludesFromSamePath($dir)
    {
        // making the reqular expression for replace
        $ct = CampTemplate::singleton();
...
ORDER BY Number DESC";
    ...
    /**
     * Retrieves a list of included resources from templates in a source path
     * @param string $tplsDir source path for templates
     * @param string $tplDir path for a template (we need both)
     */
    public function getIncludesFromTemplates( $tplsDir, $tplDir )
    {
        $returnIncludes = array();
        // get the camp template class for delimiters to use in the following regular expression
        $ct = CampTemplate::singleton();
        $reInclude = $ct->left_delimiter."\s*include.+file\s*=\s*['\"]([^'\"]+).+".$ct->right_delimiter;

        $directory = array_diff( scandir($tplsDir.DIR_SEP.$tplDir) , array("..", "."));
        // scan for template files and get includes acordingly
        foreach ($directory as $fn) {
            $theFileOrDir = $tplsDir.DIR_SEP.$tplDir.DIR_SEP.$fn;
            // template file
            if (strpos($fn, ".tpl") !== false) {
                // match include file blocks
                $tplContent = file_get_contents($theFileOrDir);
                $includeMatches = array();
                preg_match_all($reInclude, $tplContent, $includeMatches);
                foreach ($includeMatches[1] as $includeResource) {
                    // also check that the file is there and that we don't copy the same files in the template
                    if ( ( $res = realpath($tplsDir.DIR_SEP.$includeResource) ) &&
                        strpos($res, $tplsDir.DIR_SEP.$tplDir) === false
                    ) {
                        $returnIncludes[] = $res;
                    }
                }
            ...
            // traverse recursive
            if( is_dir($theFileOrDir) ) {
                $returnIncludes = array_merge($returnIncludes, $this->getIncludesFromTemplates(dirname($theFileOrDir),basename($theFileOrDir)));
            }
        }
        return array_unique($returnIncludes);
    }
    ...
    {
        $srcPath = $this->templatesPath . ( $tplDir = (empty($themeSrcDir) ? '' : '/' . $themeSrcDir) );
        $dstPath = $this->themesPath . (empty($themeSrcDir) ? '/default' : '/' . $themeSrcDir);

        $includeResources = $this->getIncludesFromTemplates(rtrim($this->templatesPath,DIR_SEP),trim($tplDir,DIR_SEP));
        foreach ($includeResources as $res) {
            $dstFile = str_replace($this->templatesPath, $dstPath, $res);
            if( !file_exists( ( $newDirName = dirname($dstFile)) ) ) {
