...
{{ list_search_results length="5" order="bypublishdate desc" constraints="type is news" }}
                ...
                <div class="post wrap">
                    ...
                    <p class="post-details">Posted on {{ $gimme->article->publish_date|camp_date_format:"%e. %M, %Y" }} by  {{*<a href="" title="Posts by {{ $gimme->article->author->name }}">*}}{{ $gimme->article->author->name }}{{*</a>*}} in <a href="{{ uri options="section" }}">{{ $gimme->section->name }}</a></p> 
