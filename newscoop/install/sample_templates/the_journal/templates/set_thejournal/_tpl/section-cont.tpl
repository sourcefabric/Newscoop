...
{{ list_articles }}
                ...
                <div class="post wrap">
                    ...
                    <p class="post-details">Published on {{ $gimme->article->publish_date|camp_date_format:"%e %M %Y" }} by  {{*<a href="{{ uri options="template author.tpl" }}" title="Posts by {{ $gimme->article->author->name }}">*}}{{ $gimme->article->author->name }}</a> in <a href="" title="View all posts in {{ $gimme->section->name }}" rel="category tag">{{ $gimme->section->name }}</a></p>
