
<body> 

<div data-role="page">
{{ include file="_tpl/jqm_header-page.tpl" }}
{{ include file="_tpl/jqm_header-front.tpl" }}
  <div data-role="content">

    <ul data-role="listview" data-inset="true">
      <li data-role="list-divider">Latest News</li>
  {{ list_articles length="5" constraints="type is news" order="byPublishDate desc" ignore_issue="true" ignore_section="true"}}
      <li>{{ strip }}
        <a href="http://{{ $gimme->publication->site }}{{ uri options="article" }}">
          {{* if $gimme->current_list->index == 1 *}} 
          <img style="background: {{ if $gimme->article->has_image(2) }}url({{uri options="image 2 width 150 height 150"}}){{ else }}url({{uri options="image 1"}} width 150 height 150"){{ /if }} no-repeat center center; width: 80px; height: 80px"  />
          {{* /if *}}
          <h3>{{* if ! $gimme->article->content_accessible }}(subscribers only){{ /if *}}
          {{ $gimme->article->name }}</h3>
          <p>{{ $gimme->article->deck }}</p>
        </a>
      {{ /strip }}</li>                                                    
  {{ /list_articles }}
    </ul>

<!-- collapsible search form -->
{{ include file="_tpl/jqm_search-form-collapsible.tpl" }}

    <ul data-role="listview" data-inset="true">
      <li data-role="list-divider">Latest Comments</li>
{{list_article_comments length="5" ignore_article="true" order="byDate desc"}}
      <li>{{ strip }}
        <a href="{{ uri options="article" }}">
          <p><strong>&raquo; {{$gimme->comment->content|truncate:100}}{{*$gimme->comment->submit_date|camp_date_format:"%H:%i"*}}</strong></p>
          <p>{{$gimme->article->name}}{{*$gimme->article->comment_count*}}</p>
        </a>
      {{ /strip }}</li>                      
{{/list_article_comments}}
    </ul>

<!-- Login / Registration / Subscriptions -->
{{ include file="_tpl/jqm_login-registration-embed.tpl" }}

<!-- navigation: all sections -->
{{ include file="_tpl/jqm_nav-sections.tpl" }}

    </div><!-- /content --> 
{{ include file="_tpl/jqm_footer.tpl" }}

</div><!-- /page -->

</body>
</html>