...
{{ /if }}
<p class="tekst"><span class="indeks"><a class="plus" href="{{ uri options="article" }}">{{ $campsite->article->name }}</a></span> from {{ $campsite->article->creation_date|camp_date_format:"%e.%m.%Y" }} in section <b>{{ $campsite->section->name }}</b></p>
{{ /list_search_results }}
