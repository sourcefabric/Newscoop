...
<!sql query "UPDATE Subscriptions SET Active=IF(Active = 'Y', 'N', 'Y') WHERE Id=?Subs">dnl
<!sql if $AFFECTED_ROWS>dnl
<!sql if @q_subs.Active == "N">dnl
	X_MSGBOX_TEXT({<LI>The subscription has been activated.</LI>})
<!sql else>dnl
	X_MSGBOX_TEXT({<LI>The subscription has been deactivated.</LI>})
<!sql endif>dnl
<!sql else>dnl
	X_MSGBOX_TEXT({<LI>Subscription status could not be changed.</LI>})
<!sql endif>dnl
	B_MSGBOX_BUTTONS
<!sql if $AFFECTED_ROWS>dnl
