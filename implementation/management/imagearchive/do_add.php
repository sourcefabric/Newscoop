...
require_once($_SERVER['DOCUMENT_ROOT']."/$ADMIN_DIR/CampsiteInterface.php"); require_once($_SERVER['DOCUMENT_ROOT']."/$ADMIN_DIR/imagearchive/include.inc.php");
...
elseif (!empty($_FILES['cImage'])) {
	...
	header('Location: '.CampsiteInterface::DisplayError(getGS("You must select an image file to upload."), $BackLink));
	exit;
}
