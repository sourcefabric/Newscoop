...
<DIV><TABLE BORDER="0">
	<TR>
		<TD VALIGN="TOP" ALIGN="RIGHT" WIDTH="150"><? putGS('Issue'); ?>: </TD>
		<TD ALIGN="LEFT">
<FORM NAME="FORM_ISS" METHOD="GET">
<? if ($NUM_ROWS) { ?>dnl
<SELECT NAME="iss" ONCHANGE="var f = this.form.iss; var v = f.options[f.selectedIndex].value; var x = 'X_ROOT/pub/issues/sections/i2.php?Language=<? pencURL($Language); ?>&Pub=<? pencURL($Pub); ?>&Issue=<? pencURL($Issue); ?>&Section=<? pencURL($Section); ?>&dstPub=<? pencURL($dstPub); ?>&dstIssue=' + v; if (v != 0) { parent.frames[1].location.href = x; }">
