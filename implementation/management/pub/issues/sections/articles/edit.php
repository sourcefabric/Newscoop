		...
		<TR>
			...
			if ($User->hasPermission('DeleteArticle')) { 
				...
				<TD class="action_link_container">
					<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1">
					<TR>
						<TD><a href="/<?php echo $ADMIN; ?>/pub/issues/sections/articles/do_del.php?Pub=<?php p($Pub); ?>&Issue=<?php p($Issue); ?>&Section=<?php p($Section); ?>&Article=<?php p($Article); ?>&Language=<?php p($Language); ?>&sLanguage=<?php p($sLanguage); ?>" onclick="return confirm('<?php putGS('Are you sure you want to delete the article $1 ($2)?', '&quot;'.javascriptspecialchars($articleObj->getTitle()).'&quot;', javascriptspecialchars($sLanguageObj->getName())); ?>');"><IMG SRC="/<?php echo $ADMIN; ?>/img/icon/delete.png" BORDER="0"></A></TD>
						<TD style="padding-left: 6px;"><a href="/<?php echo $ADMIN; ?>/pub/issues/sections/articles/do_del.php?Pub=<?php p($Pub); ?>&Issue=<?php p($Issue); ?>&Section=<?php p($Section); ?>&Article=<?php p($Article); ?>&Language=<?php p($Language); ?>&sLanguage=<?php p($sLanguage); ?>" onclick="return confirm('<?php putGS('Are you sure you want to delete the article $1 ($2)?', '&quot;'.javascriptspecialchars($articleObj->getTitle()).'&quot;', javascriptspecialchars($sLanguageObj->getName())); ?>');"><B><?php  putGS("Delete"); ?></B></A></TD>
					</TR>
					</TABLE>
				</TD>
				<?php  
