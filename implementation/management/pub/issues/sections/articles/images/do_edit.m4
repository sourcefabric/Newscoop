...
<!sql setdefault cDescription "">dnl
<!sql setdefault cPhotographer "">dnl
<!sql setdefault cPlace "">dnl
<!sql setdefault cDate "">dnl
...
<!sql if $cDescription == ""><!sql set cDescription "None"><!sql endif>dnl
<P>
B_MSGBOX({Edit image information}, {POST}, {do_edit.xql})
	X_MSGBOX_TEXT({
<!sql set AFFECTED_ROWS 0>dnl
<!sql query "UPDATE Images SET Description='?cDescription', Photographer='?cPhotographer', Place='?cPlace', Date='?cDate' WHERE IdPublication=?Pub AND NrIssue=?Issue AND NrSection=?Section AND NrArticle=?Article AND Number=?Image">dnl
<!sql if $AFFECTED_ROWS>dnl
	<LI>Image information has been updated.</LI>
X_AUDIT({43}, {Changed image properties of ~cDescription})
<!sql else>dnl
	<LI>Image information could not be updated.</LI>
<!sql endif>dnl
	})
	B_MSGBOX_BUTTONS
<!sql if $AFFECTED_ROWS>dnl
...
<!sql else>dnl
		<A HREF="X_ROOT/pub/issues/sections/articles/images/edit.xql?Pub=<!sql print #Pub>&Issue=<!sql print #Issue>&Article=<!sql print #Article>&Language=<!sql print #Language>&sLanguage=<!sql print #sLanguage>&Section=<!sql print #Section>&Image=<!sql print #Image>"><IMG SRC="X_ROOT/img/button/ok.gif" BORDER="0" ALT="OK"></A>
<!sql endif>dnl
	E_MSGBOX_BUTTONS
E_MSGBOX
<P>
