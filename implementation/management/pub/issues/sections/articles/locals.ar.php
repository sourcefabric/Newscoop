<?php 

regGS("Article automatic publishing schedule","جدول النشر الآلي للمقال");
regGS("Delete article $1","الغاء المقال $1");
regGS("$1 articles found","تم العثور على $1 مقالات");
regGS("Show article on front page","أظهر المقال في الصفحة الرئيسية");
regGS("Show article on section page","أظهار المقال في صفحة الباب");
regGS("Enter keywords, comma separated","أدخل كلمات المفتاح مستخدما الفاصلة");
regGS("You must select an article type.","اختر نوع المقال");
regGS("Article $1 added to $2. $3 from $4. $5 of $6","تمّت إضافة المقال $1 إلى $2. $3 من $4 و$5 من $6");
regGS("You do not have the right to delete articles.","لا يحق لك إلغاء المقالات");
regGS("Are you sure you want to delete the article $1 ($2)?","هل أنت متأكد من أنك تريد إلغاء المقال $1 ($2)؟");
regGS("Article $1 ($2) deleted from $3. $4 from $5. $6 ($7) of $8","تمّ إلغاء المقال $1 ($2) من$3. $4 من $5.$6 ($7) من$8");
regGS("Change article status","غيّر وضع المقال");
regGS("Change the status of article $1 ($2) from $3 to","تغيير وضع المقال $1 ($2) من $3");
regGS("You do not have the right to change this article status. Once submitted an article can only changed by authorized users.","لا يحق لك تغيير وضع المقال ولا يمكن تغيير المقال لدى إدخاله إلا من قبل المستخدمين المسموح لهم");
regGS("Article $1 status from $2. $3 from $4. $5 ($6) of $7 changed","تمّ تغيير وضع المقال $1 من $2. $3 من $4. $5  ($6) من $7");
regGS("Translate article","ترجم المقال");
regGS("You do not have the right to change this article.  You may only edit your own articles and once submitted an article can only changed by authorized users.","لا تملك صلاحية تعديل هذا المقال. يحق لك تعديل مقالاتك وعند اعتماد المقال يمكن تغيره من قبل الاشخاص المخوليين فقط");
regGS("Edit article details","تحرير المقال");
regGS("Allow users without subscriptions to view the article","اسمع للمستخدمين من دون اشتراكات رؤية هذا المقال");
regGS("The article has been locked by $1 ($2) $3 hour(s) and $4 minute(s) ago.","تم اغلاق المقال بواسطة 1$ (2$) منذ 3$ ساعات و4$ دقائق");
regGS("The article has been locked by $1 ($2) $3 minute(s) ago.","تم اغلاق المقال بواسطة 1$ (2$) منذ 3$ دقائق");
regGS("Could not save the article.  It has been locked by $1 $2 hours and $3 minutes ago.","لا يمكن حفظ المقال. لقد تم اغلاه بواسطة 1$ 2$ ساعات و3$ دقائق");
regGS("Are you sure you want to unlock it?","هل انت متأكد من فتح المقال؟");
regGS("Article is locked","المقال مغلق");
regGS("The article has been updated.","تم تحديث المقال");
regGS("The article cannot be updated or no changes have been made.","لم يحدث أي تعديل، فلم يتم تحديث المقال");
regGS("Duplicate article","كرر المقال");
regGS("The destination section is the same as the source section.","القسم الهدف هو نفس القسم المصدر");
regGS("The article is new; it is not possible to schedule it for automatic publishing.","المقال جديد. ليس من الممكن جدولته للنشر الآلي");


?>