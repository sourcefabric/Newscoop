<?

regGS("Delete article $1","Şterge articolul $1");
regGS("You do not have the right to add articles.","Nu aveţi dreptul să ştergeţi articole");
regGS("Show article on front page","Afişează articolul în pagina principală");
regGS("Show article on section page","Afişează articolul în pagina secţiunii");
regGS("Enter keywords, comma separated","Introduceţi cuvintele cheie separate de virgulă");
regGS("Adding new article","Adaug articol");
regGS("You must select an article type.","Trebuie să selectaţi tipul articolului");
regGS("The article $1 has been created","Articolul $1 a fost creat");
regGS("The article $1 could not be created","Articolul $1 poate fi creat");
regGS("Do you want to edit the article?","Doriţi să modificaţi datele articolului?");
regGS("Article $1 added to $2. $3 from $4. $5 of $6","Articolul $1 a fost adăugat la $2. $3 din $4. $5 din $6");
regGS("Delete article","Şterge articol");
regGS("You do not have the right to delete articles.","Nu aveţi dreptul să ştergeţi articole");
regGS("Are you sure you want to delete the article $1 ($2)?","Doriţi să ştergeţi articolul $1 ($2)?");
regGS("Deleting article","Şterg articol");
regGS("There are $1 image(s) left.","Mai sunt $1 imagini");
regGS("The article $1 ($2) has been deleted.","Articolul $1 ($2) a fost şters");
regGS("The article $1 ($2) could not be deleted.","Articolul $1 ($2) a fost şters");
regGS("Article $1 ($2) deleted from $3. $4 from $5. $6 ($7) of $8","Articolul $1 ($2) a fost şters din $3. $4 din $5. $6 ($7) din $8");
regGS("Change article status","Modific starea articolului");
regGS("Change the status of article $1 ($2) from $3 to","Am modificat starea articolului $1 ($2) de la $3 la");
regGS("Changing article status","Modific starea articolului");
regGS("Status of the article $1 ($2) has been changed from $3 to $4.","Starea articolului $1 ($2) a modificată de la $3 la $4 ");
regGS("Status of the article $1 ($2) could not be changed.","Starea articolului $1 ($2) nu poate fi modificată");
regGS("You do not have the right to change this article status. Once submitted an article can only changed by authorized users.","Nu aveţi dreptul să modificaţi starea articolului. Odată propus un articol poate fi modificat numai de utilizatori autorizaţi.");
regGS("Article $1 status from $2. $3 from $4. $5 ($6) of $7 changed","Starea articolului $1 din $2. $3 din $4. $5 ($6) al $7 a fost modificată");
regGS("Translate article","Traducere articol");
regGS("You do not have the right to change this article.  You may only edit your own articles and once submitted an article can only changed by authorized users.","Nu aveţi dreptul să modificaţi articolul. Puteţi să editaţi numai articolele dumneavoastră. Odată propus un articol poate fi modificat numai de utilizatori autorizaţi.");
regGS("Edit article details","Editează articolul");
regGS("Simple editor","Editor text simplu");
regGS("Advanced editor","Editor text avansat");
regGS("Allow users without subscriptions to view the article","Autorizează accesul cititorilor fără subscriere la articol");
regGS("Unlocking article","Deblochează articol");
regGS("The article has been unlocked.","Articolul a fost deblocat");
regGS("The article could not be unlocked.","Articolul nu poate fi deblocat");
regGS("You do not have the right to unlock the article. Once submitted an article can only changed by authorized users.","Nu aveţi dreptul să deblocaţi articolul. Puteţi să editaţi numai articolele dumneavoastră. Odată propus un articol poate fi modificat numai de utilizatori autorizaţi.");
regGS("Changing article details","Modificare articol");
regGS("The article has been updated.","Articolul a fost modificat");
regGS("The article cannot be updated or no changes have been made.","Articolul nu poate fi modificat sau nu au fost modificări");
regGS("Edit field: $1","Editează câmpul $1");
regGS("Preview article","Vizualizare articol");
regGS("This article cannot be previewed. Please make sure it has a <B><I>single article</I></B> template selected.","Acest articol nu poate fi vizualizat. Verificaţi daca şablonul articolului a fost selectat.");
regGS("This article has been locked by $1 ($2) at","Articolul a fost blocat de $1 ($2) în");
regGS("Now is $1","Acum este $1");
regGS("Are you sure you want to unlock it?","Doriţi să îl deblocaţi?");
regGS("Edit Article","Editează articol");


?>
