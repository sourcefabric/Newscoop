...
function upload_article_handler(&$request, &$session, &$files) {
	...
	if ($zip) {
		...
		if (!is_null($xml)) {
			...
			header("Location: /priv/pub/issues/sections/articles/edit.php?Pub=".$publication."&Issue=".$issue."&Section=".$section."&Article=".$articleNumber."&Language=".$language."&sLanguage=".$sLanguage);
		} // if (!is_null($xml))
	} // if ($zip)
