...
Define TOLContext class; used when scanning actions tree (see tol_parser.h,
...
#ifndef _TOL_CONTEXT_H
#define _TOL_CONTEXT_H
...
private:
	...
	lint2lintHash subs;							// user subscriptions
	StringHash keywords;						// keywords to search for
	string str_keywords;						// the string of keywords
	StringHash::iterator kw_i;					// keywords iterator; memorise the current element
...
public:
	...
	TOLContext();
	// copy constructor
	TOLContext(const TOLContext& c)
			: keywords(4, cpCharHashFn, cpCharEqual, stringValue)
	{
		*this = c;
	}

	int operator ==(const TOLContext& c) const;
	const TOLContext& operator =(const TOLContext&);
	void SetUserInfo(const string&, const string&);
	void SetIP(unsigned long int i)
	{
		ip = i;
	}
	void SetUser(long int u)
	{
		user_id = u;
	}
	void SetKey(unsigned long int k)
	{
		key = k;
	}
	...
	void SetKeyword(cpChar k)
	{
		keywords.insert_unique(k);
	}
	void ResetKwdIt()
	{
		kw_i = keywords.begin();
	}
	void SetStrKeywords(cpChar k)
	{
		str_keywords = k;
	}
