...
el.insert(el.end(), TOLError(id, mod));

#define FatalError(el, id, mod)\
{\
el.insert(el.end(), new TOLError(id, mod));\
return 1;\
}

#define SetPError(el, id, mod, req, l, c)\
el.insert(el.end(), new TOLError(id, mod, req, l, c));
...
// TOLError: class containing information about an error occured when parsing or writing
// output
class TOLError
{
private:
...
public:
	...
	TOLError(int p_nCode, int p_nMode, string p_coRequired = string(""),
