...
Implementation of TOLContext methods
 
******************************************************************************/

#include "tol_context.h"

const string TOLContext::emptystring = "";

// default constructor
TOLContext::TOLContext()
		: keywords(4, cpCharHashFn, cpCharEqual, stringValue)
{
	ip = 0;
	user_id = -1;
	key = 0;
...
// compare operator
int TOLContext::operator ==(const TOLContext& c) const
{
...
// assign operator
const TOLContext& TOLContext::operator =(const TOLContext& s)
{
	if (this == &s)
		return * this;
	userinfo = s.userinfo;
...
void TOLContext::SetStListStart(long int value, const string& field)
{
...
//		long int value - start element from list
//		const string& field - field (from database) to set the start element for
void TOLContext::SetStPrevStart(long int value, const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return ;
...
//		long int value - start element from list
//		const string& field - field (from database) to set the start element for
void TOLContext::SetStNextStart(long int value, const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return ;
...
//		CLevel l - list level
//		const string& field - field (from database) to set the start element for
void TOLContext::SetListStart(long int value, CLevel l, const string& field)
{
	if (l == CLV_ROOT)
		return ;
	if (l == CLV_ISSUE_LIST)
...
//		CLevel l - list level
//		const string& field - field (from database) to set the start element for
void TOLContext::SetPrevStart(long int val, CLevel l, const string& field)
{
	if (l == CLV_ROOT)
		return ;
	if (l == CLV_ISSUE_LIST)
...
//		CLevel l - list level
//		const string& field - field (from database) to set the start element for
void TOLContext::SetNextStart(long int val, CLevel l, const string& field)
{
	if (l == CLV_ROOT)
		return ;
	if (l == CLV_ISSUE_LIST)
...
void TOLContext::SetSubs(long int p, long int s)
{
	lint2lintHash::iterator p_i;
	p_i = subs.find(p);
	if (p_i == subs.end())
		subs.insert(lint2lintHash::value_type(p, lintHash(4, lintHashFn, lintEqual, lintValue)));
	p_i = subs.find(p);
	if (p_i == subs.end())
		return ;
	(*p_i).second.insert_unique(s);
}
...
//			which to append to
void TOLContext::AppendSubtitle(const string& subtitle, const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return ;
...
//			reset
void TOLContext::ResetSubtitles(const string& p_rcoField)
{
	if (p_rcoField == "")
	{
		subtitles.clear();
...
void TOLContext::SetStartSubtitle(int subtitle_nr, const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return ;
...
void TOLContext::SetAllSubtitles(bool a, const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return ;
...
void TOLContext::SetField(const string& f, const string& at)
{
	if (f == "" || at == "")
		return ;
	fields[f] = at;
...
long int TOLContext::StListStart(const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return -1;
...
// Parameters:
//		const string& field - field (article content)
long int TOLContext::StPrevStart(const string &field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return -1;
...
// Parameters:
//		const string& field - field (article content)
long int TOLContext::StNextStart(const string &field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return -1;
...
//		CLevel l - list level
//		const string& field - field (article content)
long int TOLContext::ListStart(CLevel l, const string& field)
{
	switch (l)
	{
	case CLV_ISSUE_LIST:
...
//		CLevel l - list level
//		const string& field - field (article content)
long int TOLContext::PrevStart(CLevel l, const string& field)
{
	switch (l)
	{
	case CLV_ISSUE_LIST:
...
//		CLevel l - list level
//		const string& field - field (article content)
long int TOLContext::NextStart(CLevel l, const string& field)
{
	switch (l)
	{
	case CLV_ISSUE_LIST:
...
const string& TOLContext::UserInfo(const string& attr)
{
...
// Parameters:
//		const string& attr - user attribute
bool TOLContext::IsUserInfo(const string& attr)
{
...
bool TOLContext::IsSubs(long int publication_id, long int section_id)
{
	lint2lintHash::iterator p_i = subs.find(publication_id);
	if (p_i == subs.end())
		return false;
...
// Parameters:
//		const string& field - field (article content)
int TOLContext::SubtitlesNumber(const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return 0;
...
// Parameters:
//		const string& field - field (article content)
const string& TOLContext::FieldArtType(const string& field)
{
	if (field == "")
		return emptystring;
...
// Parameters:
//		const string& field - field (article content)
const string& TOLContext::NextSubtitle(const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return emptystring;
...
// Parameters:
//		const string& field - field (article content)
const string& TOLContext::CurrentSubtitle(const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return emptystring;
...
//		const string& field - field (article content)
const string& TOLContext::SelectSubtitle(int index, const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return emptystring;
...
// Parameters:
//		const string& field - field (article content)
int TOLContext::StartSubtitle(const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return 0;
...
// Parameters:
//		const string& field - field (article content)
int TOLContext::AllSubtitles(const string& field)
{
	string actualField = (field == "" ? current_field : field);
	if (actualField == "")
		return 0;
...
void TOLContext::PrintSubs()
{
	for (lint2lintHash::iterator p_i = subs.begin(); p_i != subs.end(); ++p_i)
	{
		cout << "<p>publication: " << (*p_i).first << "\n<blockquote>";
		lintHash::iterator s_i;
