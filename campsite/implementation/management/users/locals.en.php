<?php
regGS("You do not have the right to add IP address groups.", "You do not have the right to add IP address groups.");
regGS("Readers", "Readers");
regGS("Edit reader data", "Edit reader data");
regGS("Add new IP address group", "Add new IP address group");
regGS("Staff management", "Staff management");
regGS("Subscribers management", "Subscribers management");
regGS("Edit user rights", "Edit user rights");
regGS("User Name", "User Name");
regGS("Full Name", "Full Name");
regGS("Delete user $1", "Delete user $1");
regGS("User list is empty.", "User list is empty.");
regGS("You do not have the right to create user accounts.", "You do not have the right to create user accounts.");
regGS("Title", "Title");
regGS("Gender", "Gender");
regGS("Mr.", "Mr.");
regGS("Mrs.", "Mrs.");
regGS("Ms.", "Ms.");
regGS("Dr.", "Dr.");
regGS("Male", "Male");
regGS("Female", "Female");
regGS("under 18", "under 18");
regGS("65 or over", "65 or over");
regGS("Age", "Age");
regGS("Confirm password", "Confirm password");
regGS("E-Mail", "E-Mail");
regGS("City", "City");
regGS("Street Address", "Street Address");
regGS("Postal Code", "Postal Code");
regGS("State", "State");
regGS("Phone", "Phone");
regGS("Fax", "Fax");
regGS("Contact Person", "Contact Person");
regGS("Second Phone", "Second Phone");
regGS("Employer", "Employer");
regGS("Employer Type", "Employer Type");
regGS("Corporate", "Corporate");
regGS("Non-Governmental Organisation", "Non-Governmental Organisation");
regGS("Government Agency", "Government Agency");
regGS("Academic", "Academic");
regGS("Media", "Media");
regGS("Position", "Position");
regGS("Show more user details", "Show more user details");
regGS("Hide user details", "Hide user details");
regGS("You must select a $1", "You must select a $1");
regGS("The password must be at least 6 characters long and both passwords should match.", "The password must be at least 6 characters long and both passwords should match.");
regGS("The user account could not be created.", "The user account could not be created.");
regGS("User account $1 was created successfully.", "User account $1 was created successfully.");
regGS("User account $1 created", "User account $1 created");
regGS("Click here to change password", "Click here to change password");
regGS("Old Password", "Old Password");
regGS("Password changed for $1", "Password changed for $1");
regGS("You do not have the right to change user account permissions.", "You do not have the right to change user account permissions.");
regGS("Permissions for $1 changed", "Permissions for $1 changed");
regGS("You do not have the right to change user account information.", "You do not have the right to change user account information.");
regGS("Click here to customize user permissions", "Click here to customize user permissions");
regGS("Click here to use existing user type permissions (discard customization)", "Click here to use existing user type permissions (discard customization)");
regGS("Click here to leave password unchanged", "Click here to leave password unchanged");
regGS("User account information changed for $1", "User account information changed for $1");
regGS("The password you typed is incorrect.", "The password you typed is incorrect.");
regGS("You do not have the right to delete user accounts.", "You do not have the right to delete user accounts.");
regGS("Are you sure you want to delete the user account $1 ?", "Are you sure you want to delete the user account $1 ?");
regGS("User account $1 was deleted successfully.", "User account $1 was deleted successfully.");
regGS("The user account $1 has been deleted.", "The user account $1 has been deleted.");
regGS("User IP access list management", "User IP access list management");
regGS("No records.", "No records.");
regGS("Start IP", "Start IP");
regGS("Number of addresses", "Number of addresses");
regGS("The IP Group $1 has been created.", "The IP Group $1 has been created.");
regGS("IP Group $1 added for user $2", "IP Group $1 added for user $2");
regGS("There was an error creating the IP address group.", "There was an error creating the IP address group.");
regGS("An IP address group having the $1 start address already exists.", "An IP address group having the $1 start address already exists.");
regGS("Are you sure you want to delete the IP Group $1?", "Are you sure you want to delete the IP Group $1?");
regGS("The IP address group $1 has been deleted.", "The IP address group $1 has been deleted.");
regGS("User '$1' information was changed successfully.", "User '$1' information was changed successfully.");
regGS("IP address", "IP address");
regGS("fill in from left to right at least one input box", "fill in from left to right at least one input box");
regGS("User Type", "User Type");
regGS("You must input a valid EMail address.", "You must input a valid EMail address.");
regGS("The confirm password must be at least 6 characters long and both passwords should match.", "The confirm password must be at least 6 characters long and both passwords should match.");
regGS("You must input a number between 0 and 255 into the Start IP address' $1 field.", "You must input a number between 0 and 255 into the Start IP address' $1 field.");
regGS("Delete subscriptions to $1", "Delete subscriptions to $1");
regGS("Change user account information", "Change user account information");
?>