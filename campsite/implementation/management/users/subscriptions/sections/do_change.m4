...
<!sql query "UPDATE SubsSections SET StartDate='?cStartDate', Days='?cDays', PaidDays='?cPaidDays' WHERE IdSubscription=?Subs AND SectionNumber=?Sect">dnl
<!sql if $AFFECTED_ROWS>dnl
	X_MSGBOX_TEXT({<LI>The subscription has been updated.</LI>})
<!sql else>dnl
	X_MSGBOX_TEXT({<LI>The subscription could not be updated.</LI>})
<!sql endif>dnl
	B_MSGBOX_BUTTONS
<!sql if $AFFECTED_ROWS>dnl
...
<!sql else>dnl
		<A HREF="X_ROOT/users/subscriptions/sections/change.xql?Pub=<!sql print #Pub>&User=<!sql print #User>&Subs=<!sql print #Subs>&Sect=<!sql print #Sect>"><IMG SRC="X_ROOT/img/button/ok.gif" BORDER="0" ALT="OK"></A>
<!sql endif>dnl
	E_MSGBOX_BUTTONS
E_MSGBOX
<P>
