...
<?php  if ($mia != 0) { ?>
	...
	<?php  query ("SELECT Name FROM Languages WHERE Id=".getVar($q_iss,'IdLanguage'), 'language');
	 ...
	 ?>dnl
		...
		E_LIST_ITEM
		B_LIST_ITEM(<*CENTER*>)
			<A HREF="X_ROOT/pub/issues/edit.php?Pub=<?php  pencURL($Pub); ?>&Issue=<?php  pgetUVar($q_iss,'Number'); ?>&Language=<?php  pgetUVar($q_iss,'IdLanguage'); ?>"><?php  putGS("Configure"); ?></A>
		E_LIST_ITEM
		B_LIST_ITEM(<*CENTER*>)
