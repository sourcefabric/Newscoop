B_HTML
B_DATABASE

CHECK_BASIC_ACCESS
CHECK_ACCESS({ManageSection})

B_HEAD
	X_EXPIRES
	X_TITLE({Adding New Section})
<!sql if $access == 0>dnl
	X_AD({You do not have the right to add sections.})
<!sql endif>dnl
E_HEAD

<!sql if $access>dnl
B_STYLE
E_STYLE

B_BODY

<!sql setdefault Pub 0>dnl
<!sql setdefault Issue 0>dnl
<!sql setdefault Language 0>dnl
B_HEADER({Adding New Section})
B_HEADER_BUTTONS
X_HBUTTON({Sections}, {pub/issues/sections/?Pub=<!sql print #Pub>&Issue=<!sql print #Issue>&Language=<!sql print #Language>})
X_HBUTTON({Issues}, {pub/issues/?Pub=<!sql print #Pub>})
X_HBUTTON({Publications}, {pub/})
X_HBUTTON({Home}, {home.xql})
X_HBUTTON({Logout}, {logout.xql})
E_HEADER_BUTTONS
E_HEADER

<!sql set NUM_ROWS 0>dnl
<!sql query "SELECT * FROM Issues WHERE IdPublication=?Pub AND Number=?Issue AND IdLanguage=?Language" q_iss>dnl
<!sql if $NUM_ROWS>dnl
<!sql query "SELECT * FROM Publications WHERE Id=?Pub" q_pub>dnl
<!sql if $NUM_ROWS>dnl

<!sql query "SELECT Name FROM Languages WHERE Id=?Language" q_lang>dnl
B_CURRENT
X_CURRENT({Publication:}, {<B><!sql print ~q_pub.Name></B>})
X_CURRENT({Issue:}, {<B><!sql print ~q_iss.Number>. <!sql print ~q_iss.Name> (<!sql print ~q_lang.Name>)</B>})
E_CURRENT

<!sql setdefault cName "">dnl
<!sql setdefault cNumber 0>dnl

<!sql set correct 1><!sql set created 0>dnl
<P>
B_MSGBOX({Adding new section})
	X_MSGBOX_TEXT({
<!sql if ($cName == "")>dnl
<!sql set correct 0>dnl
		<LI>You must complete the <B>Name</B> field.</LI>
<!sql endif>dnl
<!sql if ($cNumber == "")>dnl
<!sql set correct 0>dnl
<!sql setexpr cNumber ($cNumber + 0)>dnl
		<LI>You must complete the <B>Number</B> field.</LI>
<!sql endif>dnl
<!sql if $correct>dnl
<!sql set AFFECTED_ROWS 0>dnl
<!sql query "INSERT IGNORE INTO Sections SET Name='?cName', IdPublication=?Pub, NrIssue=?Issue, IdLanguage=?Language, Number=?cNumber">dnl
<!sql setexpr created ($AFFECTED_ROWS != 0)>dnl
<!sql endif>dnl
<!sql if $created>dnl
		<LI>The section <B><!sql print ~cName></B> has been successfuly added.</LI>
X_AUDIT({21}, {Section ?cName added to issue ?q_iss.Number. ?q_iss.Name (?q_lang.Name) of ?q_pub.Name})
<!sql else>dnl
<!sql if ($correct != 0)>dnl
		<LI>The section could not be added.</LI><LI>Please check if another section with the same number does not already exist.</LI>
<!sql endif>dnl
<!sql endif>dnl
		})
	B_MSGBOX_BUTTONS
<!sql if $correct && $created>dnl
		<A HREF="X_ROOT/pub/issues/sections/add.xql?Pub=<!sql print #Pub>&Issue=<!sql print #Issue>&Language=<!sql print #Language>"><IMG SRC="X_ROOT/img/button/add_another.gif" BORDER="0" ALT="Add another issue"></A>
		<A HREF="X_ROOT/pub/issues/sections/?Pub=<!sql print #Pub>&Issue=<!sql print #Issue>&Language=<!sql print #Language>"><IMG SRC="X_ROOT/img/button/done.gif" BORDER="0" ALT="Done"></A>
<!sql else>
		<A HREF="X_ROOT/pub/issues/sections/add.xql?Pub=<!sql print #Pub>&Issue=<!sql print #Issue>&Language=<!sql print #Language>"><IMG SRC="X_ROOT/img/button/ok.gif" BORDER="0" ALT="OK"></A>
<!sql endif>dnl
	E_MSGBOX_BUTTONS
E_MSGBOX
<P>

<!sql else>dnl
<BLOCKQUOTE>
	<LI>No such publication.</LI>
</BLOCKQUOTE>
<!sql endif>dnl

<!sql else>dnl
<BLOCKQUOTE>
	<LI>No such issue.</LI>
</BLOCKQUOTE>
<!sql endif>dnl

X_HR
X_COPYRIGHT
E_BODY
<!sql endif>dnl

E_DATABASE
E_HTML
