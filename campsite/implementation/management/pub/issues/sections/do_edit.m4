...
<!sql query "UPDATE Sections SET Name='?cName' WHERE IdPublication=?Pub AND NrIssue=?Issue AND Number=?Section AND IdLanguage=?Language">dnl
<!sql setexpr created ($AFFECTED_ROWS != 0)>dnl
<!sql endif>dnl
<!sql if $created>dnl
		<LI>The section <B><!sql print ~cName></B> has been successfuly modified.</LI>
X_AUDIT({21}, {Section ?cName updated to issue ?q_iss.Number. ?q_iss.Name (?q_lang.Name) of ?q_pub.Name})
<!sql else>dnl
<!sql if ($correct != 0)>dnl
		<LI>The section could not be changed.</LI>
<!--LI>Please check if another section with the same number does not already exist.</LI-->
<!sql endif>dnl
<!sql endif>dnl
		})
	B_MSGBOX_BUTTONS
...
<!sql else>
		<A HREF="X_ROOT/pub/issues/sections/edit.xql?Pub=<!sql print #Pub>&Issue=<!sql print #Issue>&Language=<!sql print #Language>&Section=<!sql print #Section>"><IMG SRC="X_ROOT/img/button/ok.gif" BORDER="0" ALT="OK"></A>
<!sql endif>dnl
	E_MSGBOX_BUTTONS
E_MSGBOX
<P>
