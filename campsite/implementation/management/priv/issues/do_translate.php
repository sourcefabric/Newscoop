...
<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" WIDTH="100%" class="page_title_container">
	<TR>
		<TD class="page_title">
		    <?php  putGS("Adding new translation"); ?>
		</TD>
...
<?php 
	...
	$correct= 0; ?>		<LI><?php  putGS('You must complete the $1 field.','<B>'.getGS('URL Name').'</B>'); ?></LI>
    <?php  }
    ...
    if ($created) { ?>		<LI><?php  putGS('The issue $1 has been successfuly added.','<B>'.encHTML(decS($cName)).'</B>' ); ?></LI>
<?php  $logtext = getGS('Issue $1 added',$cName); query ("INSERT INTO Log SET TStamp=NOW(), IdEvent=11, User='".getVar($Usr,'UName')."', Text='$logtext'"); ?>
<?php  } else {
    if ($correct != 0) { ?>		<LI><?php  putGS('The issue could not be added.'); ?></LI><LI><?php  putGS('Please check if another issue with the same number/language does not already exist.'); ?></LI>
<?php  }
} ?>		</BLOCKQUOTE></TD>
	</TR>
<?php  if ($correct && $created) { ?>	<TR>
		<TD COLSPAN="2">
		<DIV ALIGN="CENTER">
		<INPUT TYPE="button" class="button" NAME="Another" VALUE="<?php  putGS('Add another'); ?>" ONCLICK="location.href='/admin/issues/translate.php?Pub=<?php  pencURL($cPub); ?>&Issue=<?php  pencURL($cNumber); ?>'">
...
<?php  } else { ?>	<TR>
		<TD COLSPAN="2">
		<DIV ALIGN="CENTER">
		<INPUT TYPE="button" class="button" NAME="OK" VALUE="<?php  putGS('OK'); ?>" ONCLICK="location.href='/admin/issues/translate.php?Pub=<?php  pencURL($cPub); ?>&Issue=<?php  pencURL($cNumber); ?>'">
		</DIV>
		</TD>
	</TR>
<?php  } ?></TABLE></CENTER>
