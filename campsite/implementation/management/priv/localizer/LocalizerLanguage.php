    ...
    function saveFile($p_type = null)
    {
        ...
        if (is_null($p_type)) {
            if (!empty($this->m_mode)) {
        		...
        		$p_type = Localizer::GetMode();
        		if (is_null($p_type)) {
        		    return new PEAR_Error("Localizer: unknown mode");
        		}
            }
        }
        ...
        if (class_exists($className)) {
            ...
            if (method_exists($object, 'save')) {
            	...
            	return new PEAR_Error("Class $className does not have a 'save' function");
            }
        } else {
    		return new PEAR_Error("Class $className does not exist.");
        }
    } // fn saveFile
