...
<HEAD>
	...
	<TITLE><?php  putGS("Adding new alias"); ?></TITLE>
...
?>		<LI><?php  putGS('The site alias $1 has been added to publication $2.', '<B>'.$cName.'</B>', '<B>'.getHVar($q_pub,'Name').'</B>'); ?></LI>
		<?php  $logtext = getGS('The site alias $1 has been added to publication $2.',$cName,getVar($q_pub,'Name')); query ("INSERT INTO Log SET TStamp=NOW(), IdEvent=151, User='".getVar($Usr,'UName')."', Text='$logtext'"); ?>
<?php
	} else {
		if ($correct != 0) {
			if ($aliases > 0) {
