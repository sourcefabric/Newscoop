...
<!sql if $access == 0>dnl
	X_AD({You do not have the right to add new user types.})
<!sql endif>dnl
E_HEAD
...
<!sql query "INSERT IGNORE INTO UserTypes SET Name='?cName', Reader='?cReader', ManagePub='?cManagePub', DeletePub='?cDeletePub', ManageIssue='?cManageIssue', DeleteIssue='?cDeleteIssue', ManageSection='?cManageSection', DeleteSection='?cDeleteSection', AddArticle='?cAddArticle', ChangeArticle='?cChangeArticle', DeleteArticle='?cDeleteArticle', AddImage='?cAddImage', ChangeImage='?cChangeImage', DeleteImage='?cDeleteImage', ManageTempl='?cManageTempl', DeleteTempl='?cDeleteTempl', ManageUsers='?cManageUsers', ManageSubscriptions='?cManageSubscriptions', DeleteUsers='?cDeleteUsers', ManageUserTypes='?cManageUserTypes', ManageArticleTypes='?cManageArticleTypes', DeleteArticleTypes='?cDeleteArticleTypes', ManageLanguages='?cManageLanguages', DeleteLanguages='?cDeleteLanguages', ManageClasses='?cManageClasses', MailNotify='?cMailNotify', ManageDictionary='?cManageDictionary', DeleteDictionary='?cDeleteDictionary', ViewLogs='?cViewLogs' ">dnl
<!sql setexpr created ($AFFECTED_ROWS != 0)>dnl
<!sql endif>dnl
<!sql if $created>dnl
		<LI>The user type <B><!sql print ~cName></B> has been added.</LI>
X_AUDIT({121}, {User type ~cName added})
<!sql else>dnl
<!sql if ($correct != 0)>dnl
		<LI>The user type could not be added.</LI><LI>Please check if an user type with the same name does not already exist.</LI>
<!sql endif>dnl
<!sql endif>dnl
		})
	B_MSGBOX_BUTTONS
...
<!sql if $correct && $created>dnl
		<A HREF="X_ROOT/u_types/add.xql<!sql if $Back != "">?Back=<!sql print #Back><!sql endif>"><IMG SRC="X_ROOT/img/button/add_another.gif" BORDER="0" ALT="Add another user type"></A>
		<A HREF="X_ROOT/u_types/"><IMG SRC="X_ROOT/img/button/done.gif" BORDER="0" ALT="Done"></A>
<!sql else>
		<A HREF="X_ROOT/u_types/add.xql<!sql if $Back != "">?Back=<!sql print #Back><!sql endif>"><IMG SRC="X_ROOT/img/button/ok.gif" BORDER="0" ALT="OK"></A>
<!sql endif>dnl
	E_MSGBOX_BUTTONS
E_MSGBOX
<P>
