...
B_DATABASE<**>dnl
<?php
    query ("SELECT Id, IdDefaultLanguage FROM Publications WHERE Site='$_SERVER[HTTP_HOST]'", 'q_pub');
    if ($NUM_ROWS) {
 fetchRow($q_pub);
 ...
 if ($NUM_ROWS) { ?>dnl
 X_REFRESH(<*0; URL=http://<?php  p($_SERVER[HTTP_HOST]); ?><?php  pgetVar($q_iss,'FrontPage'); ?>?IdLanguage=<?php  pgetUVar($q_iss,'IdLanguage'); ?>&IdPublication=<?php  pgetUVar($q_pub,'Id'); ?>&NrIssue=<?php  pgetUVar($q_iss,'Number'); ?>*>)dnl
<?php  } else { ?>dnl
