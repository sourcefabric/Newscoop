<?php
regGS("You haven't written any articles yet.", "Нема ваших чланака.");
regGS("There are no pending items to be published.", "Нема ставки за објављивање.");
regGS("There are currently no submitted articles.", "Тренутно нема предатих чланака.");
regGS("Recently Published Articles", "Последње објављени чланци.");
regGS("Publish articles", "Објави чланак");
regGS("Publish Date", "Датум објављивања");
regGS("No articles have been published yet.", "За сада нема објављених чланака.");
regGS("Event(s)", "Догађај(и)");
regGS("Signed in: $1", "Пријављено: $1");
regGS("Help", "Помоћ");
regGS("About", "О пројекту");
regGS("Please enter your user name and password", "Унесите своје корисничко име и лозинку");
regGS("You do not have the right to access this page.", "Немате право приступа овој страни");
regGS("Login failed", "Неуспешна пријава");
regGS("Please make sure that you typed the correct user name and password.", "Проверите да ли сте исправно уписали корисничко име и лозинку");
regGS("If your problem persists please contact the site administrator $1", "Ако се проблем понови контактирајте администратора сајта");
?>