...
require_once(dirname(__FILE__)."/../include/alib/var/Alib.php");
...
//    }
    ...
    public static function GetTokensByType($type)
    {
        global $CC_CONFIG, $CC_DBC;
        $res = $CC_DBC->query(
            "SELECT CONV(token, 10, 16) AS token FROM ".$CC_CONFIG['accessTable']." WHERE type=?",
            array($type));
...
//            }
    ...
    public static function GunidFromId($p_id)
    {
        ...
        $gunid = $CC_DBC->getOne("
            SELECT CONV(gunid, 10, 16)as gunid FROM ".$CC_CONFIG['filesTable']."
            WHERE id='$p_id'
        ");
        if (PEAR::isError($gunid)) {
            return $gunid;
