...
if (!($res = mysql_query("SELECT u.Id, pu.user_id "
...
while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
	...
	$result = mysql_query("SELECT user_id FROM phorum_users WHERE fk_campsite_user_id = '"
						  . $row['Id'] . "'");
	$user = mysql_fetch_array($result, MYSQL_ASSOC);
	if (!$user) {
		// insert the corresponding phorum user
		$sql = "UPDATE phorum_users SET fk_campsite_user_id = " . $row['Id']
