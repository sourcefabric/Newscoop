...
class Archive_File
{
    ...
    public function editMetadata($p_formData)
    {
        if (!is_array($p_formData)) {
            return new PEAR_Error(getGS('Invalid parameter given to Archive_File::editMetadata()'));
        }
	...
	if ($filesize === false) {
	    return new PEAR_Error("Archive_File::OnFileUpload(): invalid parameters received.");
	}
	if (get_magic_quotes_gpc()) {
        ...
        if ($this->isValidFileType($fileName) == FALSE) {
            return new PEAR_Error("Archive_File::OnFileUpload(): invalid file type.");
        }
	...
	foreach ($result['results'] as $fileMetaData) {
	    $file = new Archive_File($fileMetaData['gunid']);
	    if ($file->exists()) {
	        $files[] = $file;
	    }
	}
				 ...
				 $p_metaData, $p_fileType)
    {
        ...
        $xmlString = Archive_File::CreateXMLTextFile($p_metaData, $p_fileType);
    ...
    /**
     * Alias of Archive_File::OnFileStore()
     *
     * @param string $p_fileName
     */
