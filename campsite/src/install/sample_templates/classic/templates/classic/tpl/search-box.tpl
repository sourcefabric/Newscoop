...
<div class="searchframebiginner">
  ...
  <div class="teaserheadinner">{{ if $campsite->language->name == "English" }}Search / Archive{{ else }}Buscar / Archivo{{ /if }}
  </div><!-- .teaserheadinner -->
  </div><!-- .teaserhead -->
...
<div id="topics">
    <a href="{{ uri options="template classic/topic.tpl" }}">{{ if $campsite->language->name == "English" }}Browse all topics{{ else }}Ver todos los temas{{ /if }}</a>
</div>

<div id="issues">
    <a href="{{ uri options="template classic/archive.tpl" }}">{{ if $campsite->language->name == "English" }}Browse all issues{{ else }}Ver todas las ediciónes{{ /if }}</a>
</div>
