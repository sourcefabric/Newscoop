...
install: dummy
	...
	install --mode=755 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/campsite_config" "$(BIN_DIR)"
	install --mode=755 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/create_instance" "$(BIN_DIR)"
	install --mode=755 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/backup_instance" "$(BIN_DIR)"
	install --mode=755 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/restore_instance" "$(BIN_DIR)"
	install --mode=755 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/remove_instance" "$(BIN_DIR)"
	install --mode=644 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/create_instance.php" "$(BIN_DIR)"
	install --mode=644 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/backup_instance.php" "$(BIN_DIR)"
	install --mode=644 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/restore_instance.php" "$(BIN_DIR)"
	install --mode=644 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/remove_instance.php" "$(BIN_DIR)"
	install --mode=644 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/cli_script_lib.php" "$(BIN_DIR)"
	install --mode=660 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/install_conf.php" "$(ETC_DIR)"
	install --mode=660 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/vhost-template.conf" "$(ETC_DIR)"
	install --mode=660 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/parser_conf.php" "$(ETC_DIR)"
	install --mode=644 --owner=$(ROOT_USER) --group=$(APACHE_GROUP) "$(INSTALL_CONF)/campsite_version.php" "$(HTML_COMMON_DIR)"
...
distclean: clean
	rm -f make.env "$(INSTALL_CONF)/campsite_config" "$(INSTALL_CONF)/configure.h"
